...
问题一将异质质量信号统一为可比较的样本与领域质量分，并在配比单纯形上刻画逐目标损失响应。质量分与人工复核结果的 Spearman 相关为 $0.666$，配比预测模型在真实留出配方上的平均 $R^2$ 为 $0.765$，表明质量评价与来源组成均包含可利用的信息。但配比排序的一致性随模型规模增大而减弱，不能将小模型的最优配方直接推广到大模型。
...
问题三在统一 FLOPs 预算下联合配置参数量、训练量、治理质量和领域配比。相对同预算的规模基线，$165$ 个情景的平均损失改善为 $4.16\%$--$5.81\%$，但其中 $162$ 个情景至少有一个评估目标退步。由此，平均最优不等于逐领域最优，实际配置还须明确任务权重与最低性能要求。
